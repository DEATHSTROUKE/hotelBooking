  ...
  return (
    <main className="main">
      ...
      <div className="rooms-container">
        ...
        {!(store.firstDate && store.lastDate && store.guestsCount) ? (
          <EmptyFieldData
            title="Для выбора комнаты заполните следующие поля"
            data={
        ...
        store.lastDate &&
        store.guestsCount &&
        store.freeRooms.length === 0 &&
        !store.isLoading ? (
          <EmptyFieldData
            title="Подходящей комнаты не найдено"
            data={
              store.guestsCount.value.count === 1
                ? "К сожалению, одноместные номера на выбранные вами даты забронированы. " +
                ...
                : store.guestsCount.value.count === 2 &&
                  store.guestsCount.value.is_family
                ? "К сожалению, семейные номера на выбранные вами даты забронированы. " +
        ...
        {store.isLoading ? (
          <EmptyFieldData title="Загрузка доступных комнат..." />
        ) : (
          ""
        )}
      </div>
